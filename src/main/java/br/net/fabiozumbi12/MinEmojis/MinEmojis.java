...
public class MinEmojis extends JavaPlugin implements Listener {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String lbl, String[] args) {
        if (args.length == 0) {
            ...
            sender.sendMessage(ChatColor.GRAY + "» " + ChatColor.ITALIC + MinEmojis.pdf.getFullName() + ", InfinityCraft");
        }
        if (args.length == 1) {
            ...
            if (args[0].equalsIgnoreCase("install") && sender instanceof Player && sender.hasPermission("minemojis.install")) {
                ...
                if (getBukkitVersion() >= 188) {
                    p.setResourcePack("https://www.dropbox.com/s/rttopwpdgcsxdzk/MinEmojis.zip?dl=0");
                } else {
                    ResourcePackAPI.setResourcepack(p, "https://www.dropbox.com/s/rttopwpdgcsxdzk/MinEmojis.zip?dl=0");
                }
